...
namespace GraphDrawer
{
    public partial class MainWindow : Window
    {
        ...
        private void Draw()
        {
            ...
            {
                x = ScaleFromPixelX(xPixel);
                y = CalculateFunctionValue(x);
                yPixel = ScaleToPixelY(y);

                nextXPixel = xPixel + 1;
                nextX = ScaleFromPixelX(nextXPixel);
                nextY = CalculateFunctionValue(nextX);
                nextYPixel = ScaleToPixelY(nextY);
        ...
        private double ScaleFromPixelX(double xPixel)
        {
        ...
        private double ScaleToPixelY(double y)
        {
