...
function loaded() {
    ...
    let buttonName = document.getElementById('button_get_person_by_name');
    buttonName.addEventListener("click", handlePersonName);
    let buttonPut = document.getElementById('put');
...
function handlePersonName(){
    ...
    fetch(`${url}?name=${name}`)
        ...
        .then((person) => {
            output.appendChild(document.createTextNode(`De name ${name} komt voor op de volgende id('s): `));
            for(let persoon of person){
                output.appendChild(document.createTextNode(` ${persoon.id}`));
            }
            console.log(person);
        })
        .catch((error) => {
